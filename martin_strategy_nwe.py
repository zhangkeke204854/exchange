...
g_params['more_position'] = False  # 币种多仓（True是，False否）
g_params['min_amplitude'] = 0.2  # 最小振幅（百分之）
g_params['max_amplitude'] = 0.6  # 最大振幅（百分之）
...
g_params['symbol_number'] = 15  # 行情触发延后次数
g_params['minutes'] = 3  # 进场延时撤单分钟数
g_params['seconds'] = 30  # 止盈止损延时撤单秒数
g_params['open_order_type'] = 2  # 开仓订单类型（1市价，2限价）
g_params['close_order_type'] = 2  # 平仓订单类型（1市价，2限价）
...
def create_signal(k_date_time, symbol, exchange_no, symbol_name, buy_position, sell_position):
    global order_dict, new_order_dict
    ...
    if create_signal_flag:
        ...
        for each in [['s', 's_flag_date_time', 's_flag_high', 's_flag_low'], ['b', 'b_flag_date_time', 'b_flag_high', 'b_flag_low']]:
            if order_dict[symbol][each[1]]:
                ...
                if close_array[-1] > flag_high > low_array[-1]:
                    ...
                    if g_params['min_amplitude'] < amplitude < g_params['max_amplitude']:
                        msg = f'当前K线时间:{k_date_time} 交易所:{exchange_no} 合约名称:{symbol_name} 当前价格:{close_array[-1]} 突破标志性k线{each[0]}的最高价:{flag_high} 振幅:{amplitude} 产生开多信号'
                        save_log(msg)
                        new_order_dict.update({symbol: {'flag': each[0], 'flag_date_time': k_date_time, 'flag_high': flag_high, 'flag_low': flag_low, 
                                                        'order_type': '开多', 'price': price, 'count': count, 'quantity': None, 'close_quantity': None, 'group': None, 'open_order_no': None, 
                                                        'close_order_no': [], 'order_message': {}, 'update_price_position': False, 'update_loss': False, 'create_order_time': None, 
                                                        'cancel_order_time': None, 'clearing_type': None, 'remove': False}})
                    else:
                        msg = f'当前K线时间:{k_date_time} 交易所:{exchange_no} 合约名称:{symbol_name} 当前价格:{close_array[-1]} 突破标志性k线{each[0]}的最高价:{flag_high} 振幅:{amplitude} 振幅不在下单区间'
                        save_log(msg)
                elif close_array[-1] < flag_low < high_array[-1]:
                    price = Q_Last(symbol)
                    count = abs(flag_high - price)  # 止盈止损点数
                    amplitude = round(count / price * 100, 2)  # 振幅
                    if g_params['min_amplitude'] < amplitude < g_params['max_amplitude']:
                        msg = f'当前K线时间:{k_date_time} 交易所:{exchange_no} 合约名称:{symbol_name} 当前价格:{close_array[-1]} 跌破标志性k线{each[0]}的最低价:{flag_low} 振幅:{amplitude} 产生开空信号'
                        save_log(msg)
                        new_order_dict.update({symbol: {'flag': each[0], 'flag_date_time': k_date_time, 'flag_high': flag_high, 'flag_low': flag_low, 
                                                        'order_type': '开空', 'price': price, 'count': count, 'quantity': None, 'close_quantity': None, 'group': None, 'open_order_no': None, 
                                                        'close_order_no': [], 'order_message': {}, 'update_price_position': False, 'update_loss': False, 'create_order_time': None, 
                                                        'cancel_order_time': None, 'clearing_type': None, 'remove': False}})
                    else:
                        msg = f'当前K线时间:{k_date_time} 交易所:{exchange_no} 合约名称:{symbol_name} 当前价格:{close_array[-1]} 跌破标志性k线{each[0]}的最低价:{flag_low} 振幅:{amplitude} 振幅不在下单区间'
                        save_log(msg)
...
def handle_data(context):
    global delay_number_dict, create_signal_dict, new_order_dict, order_dict, order_group_dict, symbol_dict, random_symbol_list, log_file
    ...
    if context.triggerType() == "S":
        # 止盈止损逻辑=====================================================================================================================================================
        for i in range(len(order_dict[symbol])):
            order_type = order_dict[symbol][i]['order_type']
            price = order_dict[symbol][i]['price']
            count = order_dict[symbol][i]['count']
            quantity = order_dict[symbol][i]['quantity']
            close_quantity = order_dict[symbol][i]['close_quantity']
            group = order_dict[symbol][i]['group']
            open_order_no = order_dict[symbol][i]['open_order_no']
            close_order_no = order_dict[symbol][i]['close_order_no']
            order_message = order_dict[symbol][i]['order_message']
            update_price_position = order_dict[symbol][i]['update_price_position']
            update_loss = order_dict[symbol][i]['update_loss']
            create_order_time = order_dict[symbol][i]['create_order_time']
            cancel_order_time = order_dict[symbol][i]['cancel_order_time']
            ...
            if open_order_no in queue_order_list:
                ...
                if order_time:
                    ...
                    if now - datetime.strptime(order_time_str, _format) >= timedelta(minutes=g_params['minutes']):
                        ...
                        if delete_order:
                            order_dict[symbol][i]['remove'] = True
                            if order_type == '开多':
                                msg = f'当前K线时间:{k_date_time} 交易所:{exchange_no} 合约名称:{symbol_name} 订单号:{open_order_no} 开多仓委托未成交，撤销委托单成功！'
                                save_log(msg)
                            else:
                                msg = f'当前K线时间:{k_date_time} 交易所:{exchange_no} 合约名称:{symbol_name} 订单号:{open_order_no} 开空仓委托未成交，撤销委托单成功！'
                                save_log(msg)
                continue
            else:
                order_status = A_OrderStatus(open_order_no)  # 指定帐户下当前商品的某个委托单的状态
                if order_status:
                    if order_status in ['7', '8', '9', 'B', 'F']:
                        if order_type == '开多':
                            msg = f'当前K线时间:{k_date_time} 交易所:{exchange_no} 合约名称:{symbol_name} 订单号:{open_order_no} 开多仓失败！'
                            save_log(msg)
                        else:
                            msg = f'当前K线时间:{k_date_time} 交易所:{exchange_no} 合约名称:{symbol_name} 订单号:{open_order_no} 开空仓失败！'
                            save_log(msg)
                        order_dict[symbol][i]['remove'] = True
                    if not update_price_position:
                        if order_status in ['5', '6']:
                            ...
                            if order_price and order_price != price:
                                msg = f'当前K线时间:{k_date_time} 交易所:{exchange_no} 合约名称:{symbol_name} 订单号:{open_order_no} 委托价:{price} 成交价:{order_price} 更新价格为实际成交价'
                                save_log(msg)
                                order_dict[symbol][i]['price'] = order_price
                            order_quantity = A_OrderFilledLot(open_order_no)  # 返回指定帐户下当前商品的某个委托单的成交数量
                            if order_quantity and order_quantity != quantity:
                                msg = f'当前K线时间:{k_date_time} 交易所:{exchange_no} 合约名称:{symbol_name} 订单号:{open_order_no} 委托量:{quantity} 成交量:{order_quantity} 更新数量为实际成交量'
                                save_log(msg)
                                order_dict[symbol][i]['quantity'] = order_quantity
                                order_dict[symbol][i]['close_quantity'] = order_quantity

                            order_dict[symbol][i]['update_price_position'] = True
                else:
                    if not update_price_position:
                        order_dict[symbol][i]['remove'] = True
            ...
            if not cancel_order_time:
                ...
                if order_type == '开多':
                    ...
                    if new_price - price_tick * g_params['profit_number'] >= profit_price:
                        profit, loss = True, False
                        last_price = new_price - price_tick * g_params['profit_number']
                        msg = f'当前K线时间:{k_date_time} 交易所:{exchange_no} 合约名称:{symbol_name} 当前价格:{new_price} 突破标志性k线{flag}的最低价:{flag_low} 满足多单止盈条件'
                        save_log(msg)
                    elif new_price - price_tick * g_params['loss_number'] <= loss_price:
                        profit, loss = False, True
                        last_price = new_price - price_tick * g_params['loss_number']
                        msg = f'当前K线时间:{k_date_time} 交易所:{exchange_no} 合约名称:{symbol_name} 当前价格:{new_price} 跌破标志性k线{flag}的最低价:{flag_low} 满足多单止损条件'
                        save_log(msg)
                else:
                    profit_price = price - count  # 止盈价格
                    loss_price = price + count  # 止损价格
                    if new_price + price_tick * g_params['profit_number'] <= profit_price:
                        profit, loss = True, False
                        last_price = new_price + price_tick * g_params['profit_number']
                        msg = f'当前K线时间:{k_date_time} 交易所:{exchange_no} 合约名称:{symbol_name} 当前价格:{new_price} 突破标志性k线{flag}的最高价:{flag_high} 满足空单止盈条件'
                        save_log(msg)
                    elif new_price + price_tick * g_params['loss_number'] >= loss_price:
                        profit, loss = False, True
                        last_price = new_price + price_tick * g_params['loss_number']
                        msg = f'当前K线时间:{k_date_time} 交易所:{exchange_no} 合约名称:{symbol_name} 当前价格:{new_price} 突破标志性k线{flag}的最高价:{flag_high} 满足空单止损条件'
                        save_log(msg)
                ...
                if profit or loss:
                    ...
                    if order_no_list:
                        order_dict[symbol][i]['close_order_no'] = order_no_list
                        order_dict[symbol][i]['cancel_order_time'] = now.strftime(_format)
            else:
                ...
                for order_no in close_order_no:
                    if order_no in queue_order_list:
                        ...
                        if order_time:
                            ...
                            if now - datetime.strptime(order_time_str, _format) >= timedelta(seconds=g_params['seconds']):
                                ...
                                if delete_order:
                                    if order_type == '开多':
                                        msg = f'当前K线时间:{k_date_time} 交易所:{exchange_no} 合约名称:{symbol_name} 订单号:{order_no} 平多仓委托未成交，撤销委托单成功！'
                                        save_log(msg)
                                    else:
                                        msg = f'当前K线时间:{k_date_time} 交易所:{exchange_no} 合约名称:{symbol_name} 订单号:{order_no} 平空仓委托未成交，撤销委托单成功！'
                                        save_log(msg)
                        continue
                    ...
                    if order_status in ['5', '6', 'A']:
                        ...
                        order_dict[symbol][i]['order_message'].update({order_no: [order_quantity, order_price, profit_loss]})

                if not delete_order_flag:
                    all_quantity = sum([value_list[0] for key, value_list in order_message.items()])
                    if all_quantity >= quantity:
                        order_dict[symbol][i]['cancel_order_time'] = None
                        order_dict[symbol][i]['remove'] = True
                    else:
                        order_dict[symbol][i]['cancel_order_time'] = None
                        if int(quantity - all_quantity) >= g_params['quantity']:
                            order_dict[symbol][i]['close_quantity'] = int(quantity - all_quantity)
                        else:
                            order_dict[symbol][i]['remove'] = True

        order_list = []
        for i in range(len(order_dict[symbol])):
            remove = order_dict[symbol][i]['remove']
            if remove:
                order_message = order_dict[symbol][i]['order_message']
                ...
                order_list.append(order_dict[symbol][i])
        ...
        if delay_number_dict[symbol] < g_params['delay_number']:
            ...
            if new_order_dict and symbol in new_order_dict:
                group_list = []
                for key_group, value_dict in order_group_dict.items():
                ...
                if len(group_list) <= 0:
                    ...
                    msg = f"当前K线时间:{k_date_time} 本次符合下单条件合约:{send_symbol_name_list} 当前组别合约数量:{len(all_position_symbol)} 大于等于指定组别数量，无法下单！"
                    save_log(msg)
                    new_order_dict = {}
                else:
                    if not random_symbol_list:
                        send_symbol_list = list(new_order_dict.keys())
                        send_symbol_name_list = [symbol_dict[send_symbol][0] for send_symbol in send_symbol_list]
                        if len(send_symbol_list) > len(group_list):
                            random_symbol_list = random.sample(send_symbol_list, len(group_list))
                            random_symbol_name_list = [symbol_dict[random_symbol][0] for random_symbol in random_symbol_list]
                        else:
                            random_symbol_list = send_symbol_list
                            random_symbol_name_list = send_symbol_name_list
                        msg = f'当前K线时间:{k_date_time} 本次符合下单条件合约:{send_symbol_name_list} 随机选中合约:{random_symbol_name_list}'
                        save_log(msg)

                    new_order_dict = {key: value for key, value in new_order_dict.items() if key in random_symbol_list}

                    if symbol in random_symbol_list:
                        order_type = new_order_dict[symbol]['order_type']

                        group = random.choice(group_list)
                        loss_number = order_group_dict[group]['当前止损次数']
                        count = order_dict[symbol]['count']
                        quantity = round(target_dict[loss_number] / count / ContractUnit(symbol))
                        ...
                        if order_flag:
                            ...
                            new_order_dict[symbol]['group'] = group
                            ...
                            order_dict[symbol].append(new_order_dict[symbol])
                        del new_order_dict[symbol]
                        random_symbol_list.remove(symbol)
