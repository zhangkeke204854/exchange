...
g_params['delay_number'] = 10  # 行情触发延后次数
g_params['minutes'] = 3  # 进场延时撤单分钟数
g_params['seconds'] = 60  # 止盈止损延时撤单秒数
...
def write_order_file():
    ...
    if json.dumps(save_order_dict, sort_keys=True) != json.dumps(order_dict, sort_keys=True):
        ...
        with open(order_file, "w", encoding="utf-8") as w:
            json.dump(save_order_dict, w, indent=2, ensure_ascii=False)
...
def write_group_file():
    ...
    if json.dumps(save_order_group_dict, sort_keys=True) != json.dumps(order_group_dict, sort_keys=True):
        ...
        with open(group_file, "w", encoding="utf-8") as w:
            json.dump(save_order_group_dict, w, indent=2, ensure_ascii=False)


# 创建信号
def create_signal(now_stamp, bar_stamp, symbol, exchange_no, symbol_name, buy_position, sell_position):
    global delay_number_dict, order_dict, new_order_dict, order_group_dict
                ...
                if each[0] == 's' and g_params['s_buy'] == 1:
                    if close_array[-1] > flag_high > low_array[-1]:
                        ...
                        if min_amplitude < amplitude < max_amplitude:
                            ...
                            new_order_dict.update({symbol: {'flag': each[0], 'flag_date_time': bar_stamp, 'flag_high': flag_high, 'flag_low': flag_low, 
                            ...
                            msg = f'当前时间:{now_stamp} Bar时间:{bar_stamp} 交易所:{exchange_no} 合约名称:{symbol_name} 当前价格:{close_array[-1]} 突破标志性k线{each[0]}的最高价:{flag_high} 振幅:{amplitude} 组别:{group} 产生开多信号'
                            save_log(msg)
                            break
                        else:
                            msg = f'当前时间:{now_stamp} Bar时间:{bar_stamp} 交易所:{exchange_no} 合约名称:{symbol_name} 当前价格:{close_array[-1]} 突破标志性k线{each[0]}的最高价:{flag_high} 振幅:{amplitude} 组别:{group} 振幅不在下单区间'
                            save_log(msg)
                    ...
                    if close_array[-1] < flag_low < high_array[-1]:
                        ...
                        if min_amplitude < amplitude < max_amplitude:
                            ...
                            new_order_dict.update({symbol: {'flag': each[0], 'flag_date_time': bar_stamp, 'flag_high': flag_high, 'flag_low': flag_low, 
                                                            ...
                                                            'open_order_no': None, 'close_order_no': [], 'order_message': {}, 'update_price_position': False, 
                                                            'create_order_time': None, 'cancel_order_time': None, 'remove': False}})
                            msg = f'当前时间:{now_stamp} Bar时间:{bar_stamp} 交易所:{exchange_no} 合约名称:{symbol_name} 当前价格:{close_array[-1]} 跌破标志性k线{each[0]}的最低价:{flag_low} 振幅:{amplitude} 组别:{group} 产生开空信号'
                            save_log(msg)
                            break
                        else:
                            msg = f'当前时间:{now_stamp} Bar时间:{bar_stamp} 交易所:{exchange_no} 合约名称:{symbol_name} 当前价格:{close_array[-1]} 跌破标志性k线{each[0]}的最低价:{flag_low} 振幅:{amplitude} 组别:{group} 振幅不在下单区间'
                            save_log(msg)
    ...
    if close_array[-1] > max_high:
        order_dict[symbol].update({'s_flag_date_time': bar_stamp, 
                                    's_flag_open': open_array[-1], 
                                    's_flag_high': high_array[-1], 
                                    's_flag_low': low_array[-1], 
                                    's_flag_close': close_array[-1]})
        msg = f'当前时间:{now_stamp} Bar时间:{bar_stamp} 交易所:{exchange_no} 合约名称:{symbol_name} 形成标志性k线s，开盘价:{open_array[-1]} 最高价:{high_array[-1]} 最低价:{low_array[-1]} 收盘价:{close_array[-1]}'
        save_log(msg)
    
    elif close_array[-1] < min_low:
        order_dict[symbol].update({'b_flag_date_time': bar_stamp, 
                                    'b_flag_open': open_array[-1], 
                                    'b_flag_high': high_array[-1], 
                                    'b_flag_low': low_array[-1], 
                                    'b_flag_close': close_array[-1]})
        msg = f'当前时间:{now_stamp} Bar时间:{bar_stamp} 交易所:{exchange_no} 合约名称:{symbol_name} 形成标志性k线b，开盘价:{open_array[-1]} 最高价:{high_array[-1]} 最低价:{low_array[-1]} 收盘价:{close_array[-1]}'
        save_log(msg)
...
def send_order(account, symbol, exchange_no, symbol_name, send_order_list, enum_order='2'):
    ...
    now_stamp = send_order_list[0]
    bar_stamp = send_order_list[1]
    order_type = send_order_list[2]
    quantity = send_order_list[3]
    price = send_order_list[4]
    ...
    if order_flag:
        if enum_order == '1':
            msg = f"当前时间:{now_stamp} Bar时间:{bar_stamp} 交易所:{exchange_no} 合约名称:{symbol_name} {order_type}仓，市价委托成功 价格:{price} 数量:{quantity} 订单号:{session_id}" 
        else:
            msg = f"当前时间:{now_stamp} Bar时间:{bar_stamp} 交易所:{exchange_no} 合约名称:{symbol_name} {order_type}仓，限价委托成功 价格:{price} 数量:{quantity} 订单号:{session_id}" 
    else:
        if enum_order == '1':
            msg = f"当前时间:{now_stamp} Bar时间:{bar_stamp} 交易所:{exchange_no} 合约名称:{symbol_name} {order_type}仓，没有检测到开仓委托订单号，市价委托失败"
        else:
            msg = f"当前时间:{now_stamp} Bar时间:{bar_stamp} 交易所:{exchange_no} 合约名称:{symbol_name} {order_type}仓，没有检测到开仓委托订单号，限价委托失败" 
...
def handle_data(context):
    ...
    now = datetime.now()
    now_stamp = now.strftime(_format)
    ...
    if symbol not in symbol_dict:
        symbol_name = SymbolName(symbol)  # 合约名称
        exchange_no = ExchangeName(symbol)  # 交易所代码
        symbol_dict.update({symbol: [symbol_name, exchange_no]})
    symbol_name = symbol_dict[symbol][0]
    exchange_no = symbol_dict[symbol][1]
    ...
    # date_time_stamp = str(context.dateTimeStamp())  # 获取当前触发的时间戳
    # trigger_stamp = f"{date_time_stamp[:4]}-{date_time_stamp[4:6]}-{date_time_stamp[6:8]} {date_time_stamp[8:10]}:{date_time_stamp[10:12]}:{date_time_stamp[12:14]}"

    date_str = str(Date(symbol, g_params['barType'], g_params['barInterval']))  # 当前Bar的日期
    time_str = '%06d' % int(Time(symbol, g_params['barType'], g_params['barInterval']) * 1000000)  # 当前Bar的时间
    bar_stamp = f"{date_str[:4]}-{date_str[4:6]}-{date_str[6:8]} {time_str[:2]}:{time_str[2:4]}:{time_str[4:]}"

    if context.triggerType() == "K" and not create_signal_dict[symbol]:
        create_signal(now_stamp, bar_stamp, symbol, exchange_no, symbol_name, buy_position, sell_position)  # 创建信号
    ...
    if context.triggerType() == "S":
        ...
        for i in range(len(order_dict[symbol]['order'])):
                ...
                if order_time:
                    ...
                    if now - datetime.strptime(order_time_str, _format) >= timedelta(minutes=g_params['minutes']):
                        ...
                        if delete_order:
                            ...
                            if order_type == '开多':
                                msg = f'当前时间:{now_stamp} Bar时间:{bar_stamp} 交易所:{exchange_no} 合约名称:{symbol_name} 组别:{group} 订单号:{open_order_no} 开多仓委托未成交，撤销委托单成功！'
                                save_log(msg)
                            else:
                                msg = f'当前时间:{now_stamp} Bar时间:{bar_stamp} 交易所:{exchange_no} 合约名称:{symbol_name} 组别:{group} 订单号:{open_order_no} 开空仓委托未成交，撤销委托单成功！'
                                save_log(msg)
                continue
            else:
                    ...
                    if order_status in ['0', '1', '2', '3', '4', '7', '8', '9', 'B', 'F']:
                        ...
                        if order_time:
                            ...
                            if now - datetime.strptime(order_time_str, _format) >= timedelta(minutes=g_params['minutes']):
                                ...
                                if delete_order:
                                    order_dict[symbol]['order'][i]['remove'] = True
                                    if order_type == '开多':
                                        msg = f'当前时间:{now_stamp} Bar时间:{bar_stamp} 交易所:{exchange_no} 合约名称:{symbol_name} 组别:{group} 订单号:{open_order_no} 开多仓委托成交失败，撤销委托单成功！'
                                        save_log(msg)
                                    else:
                                        msg = f'当前时间:{now_stamp} Bar时间:{bar_stamp} 交易所:{exchange_no} 合约名称:{symbol_name} 组别:{group} 订单号:{open_order_no} 开空仓委托成交失败，撤销委托单成功！'
                                        save_log(msg)
                        else:
                            order_dict[symbol]['order'][i]['remove'] = True
                            if order_type == '开多':
                                msg = f'当前时间:{now_stamp} Bar时间:{bar_stamp} 交易所:{exchange_no} 合约名称:{symbol_name} 组别:{group} 订单号:{open_order_no} 开多仓委托成交失败！'
                                save_log(msg)
                            else:
                                msg = f'当前时间:{now_stamp} Bar时间:{bar_stamp} 交易所:{exchange_no} 合约名称:{symbol_name} 组别:{group} 订单号:{open_order_no} 开空仓委托成交失败！'
                                save_log(msg)
                        ...
                        if not update_price_position:
                            ...
                            if order_price and order_price != price:
                                msg = f'当前时间:{now_stamp} Bar时间:{bar_stamp} 交易所:{exchange_no} 合约名称:{symbol_name} 组别:{group} 订单号:{open_order_no} 委托价:{price} 成交价:{order_price} 更新价格为实际成交价'
                                save_log(msg)
                            ...
                            if order_quantity and order_quantity != quantity:
                                msg = f'当前时间:{now_stamp} Bar时间:{bar_stamp} 交易所:{exchange_no} 合约名称:{symbol_name} 组别:{group} 订单号:{open_order_no} 委托量:{quantity} 成交量:{order_quantity} 更新数量为实际成交量'
                                save_log(msg)
            ...
            if order_type == '开多':
                if buy_position == 0:
                    order_dict[symbol]['order'][i]['remove'] = True
                    auto_flag = False
                elif buy_position < close_quantity:
                    order_dict[symbol]['order'][i]['close_quantity'] = buy_position
            else:
                if sell_position == 0:
                    order_dict[symbol]['order'][i]['remove'] = True
                    auto_flag = False
                elif sell_position < close_quantity:
                    order_dict[symbol]['order'][i]['close_quantity'] = sell_position

            if not auto_flag:
                all_order_no = A_AllOrderNo(symbol, g_params['account'])  # 指定合约指定账户所有定单号的列表
                order_no = all_order_no[-1]
                order_status = A_OrderStatus(order_no)  # 指定帐户下当前商品的某个委托单的状态
                if order_status in ['5', '6', 'A']:
                    order_filled_list =  A_OrderFilledList(order_no)  # 定单的成交情况信息
                    if order_filled_list:
                        direct, offset = order_filled_list[0]['Direct'], order_filled_list[0]['Offset']
                        match_price, match_qty = order_filled_list[0]['MatchPrice'], order_filled_list[0]['MatchQty']
                        if order_type == '开多':
                            if direct == 'S' and offset in ['C', 'T']:
                                profit_loss = round(float((match_price - price) * ContractUnit(symbol) * match_qty), 2)
                                order_dict[symbol]['order'][i]['order_message'].update({order_no: [match_qty, match_price, profit_loss]})
                        else:
                            if direct == 'B' and offset in ['C', 'T']:
                                profit_loss = round(float((price - match_price) * ContractUnit(symbol) * match_qty), 2)
                                order_dict[symbol]['order'][i]['order_message'].update({order_no: [match_qty, match_price, profit_loss]})
            else:
                if not cancel_order_time:
                    ...
                    if order_type == '开多':
                        ...
                        if new_price >= profit_price + price_tick * g_params['profit_number']:
                            ...
                            msg = f'当前时间:{now_stamp} Bar时间:{bar_stamp} 交易所:{exchange_no} 合约名称:{symbol_name} 组别:{group} 当前价格:{new_price} 突破标志性k线{flag}的最低价:{flag_low} 满足多单止盈条件'
                            save_log(msg)
                        elif new_price <= loss_price - price_tick * g_params['loss_number']:
                            profit, loss = False, True
                            msg = f'当前时间:{now_stamp} Bar时间:{bar_stamp} 交易所:{exchange_no} 合约名称:{symbol_name} 组别:{group} 当前价格:{new_price} 跌破标志性k线{flag}的最低价:{flag_low} 满足多单止损条件'
                            save_log(msg)
                    else:
                        profit_price = price - count  # 止盈价格
                        loss_price = price + count  # 止损价格
                        if new_price <= profit_price - price_tick * g_params['profit_number']:
                            profit, loss = True, False
                            msg = f'当前时间:{now_stamp} Bar时间:{bar_stamp} 交易所:{exchange_no} 合约名称:{symbol_name} 组别:{group} 当前价格:{new_price} 突破标志性k线{flag}的最高价:{flag_high} 满足空单止盈条件'
                            save_log(msg)
                        elif new_price >= loss_price + price_tick * g_params['loss_number']:
                            profit, loss = False, True
                            msg = f'当前时间:{now_stamp} Bar时间:{bar_stamp} 合约名称:{symbol_name} 组别:{group} 当前价格:{new_price} 突破标志性k线{flag}的最高价:{flag_high} 满足空单止损条件'
                            save_log(msg)
                    ...
                    if profit or loss:
                        ...
                        if exchange_no in ['SHFE', 'INE']:
                            ...
                            if yesterday_position > 0:
                                if yesterday_position >= close_quantity:
                                    if order_type == '开多':
                                        send_order_list = [now_stamp, bar_stamp, '平多', close_quantity, new_price]
                                    else:
                                        send_order_list = [now_stamp, bar_stamp, '平空', close_quantity, new_price]
                                    ...
                                    if order_type == '开多':
                                        send_order_list = [now_stamp, bar_stamp, '平多', yesterday_position, new_price]
                                    else:
                                        send_order_list = [now_stamp, bar_stamp, '平空', yesterday_position, new_price]
                                    order_flag, session_id = send_order(g_params['account'], symbol, exchange_no, symbol_name, send_order_list, enum_order)
                                    if order_flag:
                                        order_no_list.append(session_id)
                                    
                                    if order_type == '开多':
                                        send_order_list = [now_stamp, bar_stamp, '平多今', close_quantity - yesterday_position, new_price]
                                    else:
                                        send_order_list = [now_stamp, bar_stamp, '平空今', close_quantity - yesterday_position, new_price]
                                    order_flag, session_id = send_order(g_params['account'], symbol, exchange_no, symbol_name, send_order_list, enum_order)
                                    if order_flag:
                                        order_no_list.append(session_id)
                            else:
                                if order_type == '开多':
                                    send_order_list = [now_stamp, bar_stamp, '平多今', close_quantity, new_price]
                                else:
                                    send_order_list = [now_stamp, bar_stamp, '平空今', close_quantity, new_price]
                                order_flag, session_id = send_order(g_params['account'], symbol, exchange_no, symbol_name, send_order_list, enum_order)
                                if order_flag:
                                    order_no_list.append(session_id)
                        else:
                        ...
                        if order_no_list:
                            ...
                            order_dict[symbol]['order'][i]['cancel_order_time'] = now_stamp
                else:
                        ...
                        if order_no in queue_order_list:
                            ...
                            if order_time:
                                ...
                                if now - datetime.strptime(order_time_str, _format) >= timedelta(seconds=g_params['seconds']):
                                    ...
                                    if delete_order:
                                        if order_type == '开多':
                                            msg = f'当前时间:{now_stamp} Bar时间:{bar_stamp} 交易所:{exchange_no} 合约名称:{symbol_name} 组别:{group} 订单号:{order_no} 平多仓委托未成交，撤销委托单成功！'
                                            save_log(msg)
                                        else:
                                            msg = f'当前时间:{now_stamp} Bar时间:{bar_stamp} 交易所:{exchange_no} 合约名称:{symbol_name} 组别:{group} 订单号:{order_no} 平空仓委托未成交，撤销委托单成功！'
                                            save_log(msg)
                            continue
        ...
        for i in range(len(order_dict[symbol]['order'])):
            ...
            if remove:
                ...
                group = order_dict[symbol]['order'][i]['group']
                if profit_loss > 0:
                    order_group_dict[group]['盈利次数'] += 1
                    order_group_dict[group]['盈利金额'] += profit_loss
                    order_group_dict[group]['盈亏总额'] += profit_loss
                    order_group_dict[group]['当前止损次数'] = 0
                elif profit_loss < 0:
                    order_group_dict[group]['亏损次数'] += 1
                    order_group_dict[group]['亏损金额'] += profit_loss 
                    order_group_dict[group]['盈亏总额'] += profit_loss
        ...
        if delay_number_dict[symbol] < g_params['delay_number']:
        ...
        elif start_create_time_1 <= now <= end_create_time_1:
            pass
        elif start_create_time_2 <= now <= end_create_time_2:
            pass
        elif start_create_time_3 <= now <= end_create_time_3:
            pass
        elif start_create_time_4 <= now <= end_create_time_4:
            pass
        else:
            create_signal_dict[symbol] = False
            ...
            if new_order_dict and symbol in new_order_dict:
                if not random_symbol:
                    ...
                    msg = f'当前时间:{now_stamp} Bar时间:{bar_stamp} 本次符合下单条件合约:{send_symbol_name_list} 随机选中合约:{random_symbol_name} 交易所:{random_symbol_exchange}'
                    save_log(msg)
                ...
                if random_symbol == symbol:
                    ...
                    send_order_list = [now_stamp, bar_stamp, order_type, quantity, price]
                    if exchange_no in ['SHFE', 'INE']:
                        enum_order = '2'
                    else:
                    ...
                    if order_flag:
                        ...
                        new_order_dict[symbol]['create_order_time'] = now_stamp
